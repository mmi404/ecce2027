"""
YOLO26s replicate of run_wp4_mode_groups.py: the same E1-vs-E2 headline per-class and
mode-group comparison on ctg_pooled_eval, but for the YOLO26s runs -- this is the
architecture-robustness check (does the class-concentrated transfer gap found for
YOLOv8s persist under Ultralytics' current flagship model?). Point estimates only, no
bootstrap CI (kept fast/compact; the statistical case for the gap itself is already
established by the YOLOv8s CIs in transfer_gap.csv -- this checks whether the same
*pattern* replicates, not whether YOLO26s's own gap is non-zero).
"""
import sys
import os
import csv
...
def main():
    thresholds = {}
    with open(os.path.join(RESULTS_DIR, 'thresholds_yolo26.csv')) as f:
        for row in csv.DictReader(f):
            thresholds[row['run_id']] = float(row['threshold'])

    image_ids, gt_by_class = w.load_gt('ctg_pooled_eval')
    e1_preds = [w.load_preds(f'e1_{k}_y26', 'ctg_pooled_eval') for k in range(3)]
    e1_thresholds = [thresholds[f'e1_{k}_y26'] for k in range(3)]

    e2_fold_preds = [w.load_preds(f'e2_{k}_y26', f'ctg_fold{k}_eval') for k in range(3)]
    ...
    with open(os.path.join(RESULTS_DIR, 'headline_per_class_point_yolo26.csv'), 'w', newline='') as f:
        fieldnames = list(per_class_rows[0].keys())
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(per_class_rows)
    print('Wrote results/headline_per_class_point_yolo26.csv')
    ...
    with open(os.path.join(RESULTS_DIR, 'mode_group_summary_yolo26.csv'), 'w', newline='') as f:
        fieldnames = list(group_rows[0].keys())
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(group_rows)
    print('\nWrote results/mode_group_summary_yolo26.csv')
    for r in group_rows:
